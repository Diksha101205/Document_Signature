...
function App() {
  ...
  const saveSignaturePlaceholder = async (event) => {
    ...
    if (!signerEmail.trim()) {
      setError('Signer email is required before placing a signature.')
      return
    }
    ...
    const width = 180
    const height = 56
    const x = Math.max(0, Math.round(event.clientX - rect.left - width / 2))
    const y = Math.max(0, Math.round(event.clientY - rect.top - height / 2))
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/docs/${selectedDocument.id}/signatures`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token.trim()}`,
      ...
      <section className="mx-auto w-full max-w-7xl px-5 py-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <section className="rounded-lg border border-slate-200 bg-white">
            ...
            <div className="grid gap-3 border-b border-slate-100 px-5 py-4 sm:grid-cols-[1fr_1fr_auto]">
              <input
              ...
              <input
                ...
                value={signerEmail}
              />
              <span className="grid h-10 place-items-center rounded-lg bg-slate-100 px-3 text-xs font-semibold text-slate-600">
                {isSavingSignature ? 'Saving...' : 'Click PDF to place'}
              </span>
            </div>
